    ...
    <Card className={className}>
      <CardHeader className='flex flex-row items-center justify-between pb-3'>
        <CardTitle className='flex items-center gap-2 text-base'>
          ...
          {title}
          <span className='text-sm font-normal text-muted-foreground'>({sessionName})</span>
        </CardTitle>
        <div className='flex items-center gap-2'>
          ...
          >
            <StatusIcon className={cn('mr-1 h-3 w-3', (status === 'connecting' || status === 'reconnecting') && 'animate-spin')} />
            {statusInfo.label}
          </Badge>
          {expandable && (
